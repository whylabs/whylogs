...
class MatplotlibProfileVisualizer(BaseProfileVisualizer):
    ...
    def _chart_theming():
        ...
        ax = _plt.axes()
        fig.text(1.0, 1.06, 'Made with WhyLogs',
    ...
    def plot_distribution(self, variable, variant='auto', **kwargs):
        ...
        # Fill areas
        ax.fill_between(chart_data.loc[:, 'date'],
        ...
        ax.set_title(f"Distribution - Estimated Quantiles ({variable})",
                     loc='left', fontweight='bold')
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, loc='upper center',
                  bbox_to_anchor=(0.5, -0.1), frameon=False, ncol=3)
    ...
    def plot_missing_values(self, variable, variant='auto', **kwargs):
        ...
        chart_data = self._summary_data_preprocessing(variable)
        chart_data.loc[:, 'mv_ratio'] = chart_data.loc[:,
                                        'type_null_count'] / chart_data.loc[:,
                                                             'count']
        ...
        ax.yaxis.set_label_text("Missing Value to Total Count Ratio",
                                fontweight='bold')
        ax.set_title(f"Missing Values ({variable})", loc='left',
                     fontweight='bold')
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, loc='upper center',
                  bbox_to_anchor=(0.5, -0.1), frameon=False, ncol=3)
    ...
    def plot_uniqueness(self, variable, variant='auto', **kwargs):
        ...
        if (chart_data.loc[:, 'nunique_numbers'].sum() == 0 and
                chart_data.loc[:, 'nunique_str'].sum() == 0):
            print("No data appropriate for uniqueness plot.")
            return
        elif (chart_data.loc[:, 'nunique_numbers'].sum() >
              chart_data.loc[:, 'nunique_str'].sum()):
            metrics = ['nunique_numbers', 'nunique_numbers_lower',
        ...
        ax.set_title(f"Estimated Unique Values ({variable})", loc='left',
                     fontweight='bold')
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, loc='upper center',
                  bbox_to_anchor=(0.5, -0.1), frameon=False, ncol=3)
    ...
    def plot_data_types(self, variable, **kwargs):
        ...
        type_metrics = {"Unknown": 'type_unknown_count',
        ...
        ax.set_title(f"Inferred Data Type ({variable})", loc='left',
                     fontweight='bold')
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, loc='upper center',
                  bbox_to_anchor=(0.5, -0.1), frameon=False, ncol=3)
