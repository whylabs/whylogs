...
class PIITracker:
    def __init__(self, column_name, reporting_threshold=1):
        ...
        self.pii_constraints = ValueConstraints([
                containsEmailConstraint(
                    name=f"The column {self.column_name} contains some values identified as e-mail addresses"
                ),
                containsCreditCardConstraint(
                    name=f"The column {self.column_name} contains some values identified as credit card numbers"
                ),
                containsSSNConstraint(
                    name=f"The column {self.column_name} contains some values identified as social security numbers"
                ),
            ])
        self.tracking = True
