...
class DatasetProfile:
    ...
    def add_metric_plugin(self, metric: MetricPlugin):
        ...
        if metric.target_column_name:
            if metric.target_column_name in self._column_metric_plugins.keys():
        ...
        if column_name in self._column_metric_plugins.keys():
            for plugin_name in self._column_metric_plugins[column_name]:
                if plugin_name not in self._metric_plugins.keys():
                    logger.warning(f"Metric plugin ('{plugin_name}') not found in '{self._metric_plugins}'")
                else:
                    self._metric_plugins[plugin_name].track(data)
    ...
    def to_summary(self):
        ...
        if self.metric_plugins.items():
            ...
            for key in self.metric_plugins.keys():
                ...
                metric_plugin_summaries[key] = plugin_summary

            summary = DatasetSummaryV2(
                properties=self.to_properties(),
                columns=column_summaries,
                metric_plugins=metric_plugin_summaries,
            )
        else:
            # metric_plugin_summaries = {name: metric_plugin.to_protobuf() for name, metric_plugin in self.metric_plugins.items()}
            summary = DatasetSummaryV2(
                properties=self.to_properties(),
                columns=column_summaries,
            )
