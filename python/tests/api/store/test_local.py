...
class TestLocalStore(object):
    ...
    def test_write(self, store, profile_view):
        ...
        store.write(profile_view=profile_view, dataset_id="test_name")
    ...
    def test_get_by_date(self, store, profile_view):
        ...
        query = DateQuery(start_date=datetime.now(timezone.utc), dataset_id="test_name")
    ...
    def test_get_by_dataset_id(self, store, profile_view):
        store.write(profile_view=profile_view, dataset_id="test_name")
        query = DatasetIdQuery(dataset_id="test_name")
    ...
    def test_get_ignores_files_that_dont_match_pattern(self, store, profile_view):
        store.write(profile_view=profile_view, dataset_id="test_name")
        query = DateQuery(start_date=datetime.now(timezone.utc), dataset_id="test_name")
        Path(os.path.join(store._default_path, query.dataset_id, "profile_2022-02-01_23123.bin")).touch()
