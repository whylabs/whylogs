...
@pytest.mark.skip("Haven't figured out how to curry Polars expressions yet")
def test_filtered_single_column_segment() -> None:
    ...
    test_segments[segment_column].filter = SegmentFilter(filter_function=(pl.col("col1") > 49))
    ...
    first_segment: Segment = next(next(iter(segments)))  # polars comes out in different order
    first_segment_profile = results.profile(first_segment)
    assert first_segment.key == ("x0",)
    assert first_segment_profile is not None
    assert first_segment_profile._columns["col1"]._schema.dtype == pl.Int64  # np.int64
...
def test_segment_write_roundtrip_versions(tmp_path: Any, v0) -> None:
    ...
    seg_i = iter(segments)  # polars segments order is non-deterministic
    first_segment: Segment = next(seg_i)
    if first_segment.key == ("x1",):
        first_segment = next(seg_i)
    first_segment_profile = results.profile(first_segment)
    assert first_segment.key == ("x0",)
    assert first_segment_profile is not None
    assert first_segment_profile._columns["col1"]._schema.dtype == pl.Int64  # np.int64
...
def test_segment_with_nans() -> None:
    df = pl.DataFrame({"col_1": [1, 2, 3, 4, 5, 6], "col_nan": [0.0, 0.0, None, None, np.nan, math.nan]})
    ...
    assert profile_results.count == 3  # col_nan = True
