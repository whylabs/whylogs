...
from whylogs.datasets.configs import EmployeeConfig

dataset_config = EmployeeConfig()


class TestEmployeeDataset(object):
    @pytest.fixture
    def dataset(self):
        return Employee(version="base")
    ...
    def test_non_existing_version(self):
        with pytest.raises(ValueError, match="Version not found in list of available versions."):
            Employee(version="nonexisting-version")
    ...
    def test_datetime_baseline_ts(self, dataset):
        dataset.set_parameters("1d", production_start_timestamp=datetime.now(), baseline_timestamp=datetime.now())
        assert isinstance(dataset.baseline_timestamp, date)
    ...
    def test_get_production_by_date(self, dataset):
        dataset.set_parameters("1d", production_start_timestamp=datetime.now(timezone.utc).date())
        batch = dataset.get_production_data(target_date=datetime.now(timezone.utc).date())
        ...
        batch = dataset.get_production_data(target_date=datetime.now())
        assert batch.timestamp.day == datetime.now(timezone.utc).day
        assert batch.timestamp.month == datetime.now(timezone.utc).month
        assert batch.timestamp.year == datetime.now(timezone.utc).year
        assert len(batch.data) > 0

    def test_get_production_pass_both_arguments(self, dataset):
        dataset.set_parameters("1d", production_start_timestamp=datetime.now(timezone.utc).date())
        with pytest.raises(ValueError, match="Either date or number_batches should be passed, not both."):
            dataset.get_production_data(target_date=datetime.now(timezone.utc).date(), number_batches=30)

    def test_get_production_pass_no_arguments(self, dataset):
        dataset.set_parameters("1d", production_start_timestamp=datetime.now(timezone.utc).date())
        with pytest.raises(ValueError, match="date or number_batches must be passed to get_production_data."):
            dataset.get_production_data()

    def test_get_production_wrong_date_type(self, dataset):
        dataset.set_parameters("1d", production_start_timestamp=datetime.now(timezone.utc).date())
        with pytest.raises(ValueError, match="Target date should be either of date or datetime type."):
            dataset.get_production_data(target_date="2022-02-12")

    def test_get_production_without_set_parameters(self, dataset):
        batches = dataset.get_production_data(number_batches=3)
        assert len(list(batches)) == 3

    def test_get_production_valid_interval(self, dataset):
        dataset.set_parameters("3d")
        batches = dataset.get_production_data(number_batches=3)
        assert len(list(batches)) == 3

    def test_get_production_invalid_interval(self, dataset):
        with pytest.raises(ValueError, match="Could not parse interval {}!".format("12y")):
            dataset.set_parameters("12y")
        with pytest.raises(
            ValueError, match="Maximum allowed interval for this dataset is {}".format(EmployeeConfig.max_interval)
        ):
            dataset.set_parameters("120d")
        with pytest.raises(
            ValueError, match="Current accepted unit for this dataset is {}".format(EmployeeConfig.base_unit)
        ):
            dataset.set_parameters("5M")

    def test_production_df_date_matches_config_timestamp(self, dataset):
        dataset.set_parameters(original=True)
        assert dataset.production_df.iloc[0].name == dataset_config.production_start_timestamp["base"]

    def test_original_parameter_overrides_timestamps(self, dataset):
        dataset.set_parameters(
            baseline_timestamp=datetime.now(timezone.utc).date(),
            production_start_timestamp=datetime.now(timezone.utc).date(),
            original=True,
        )
        assert dataset.production_df.iloc[0].name == dataset_config.production_start_timestamp["base"]
