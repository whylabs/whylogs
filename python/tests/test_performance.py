...
def test_track_column_benchmark() -> None:
    ...
    for i in range(len(row_counts)):
        full_df = pd.DataFrame(
            (np.random.rand(row_counts[i], 1) - 0.5) * 3, columns=["A"]
        )
        ...
        profiler.enable()
        col_prof = ColumnProfile(ColumnSchema("perf_test", NumericColumnType[int](int)))
        col_prof.track_column(full_df["A"])
        profiler.disable()
        stats = pstats.Stats(profiler, stream=string_output_stream).sort_stats(
            "cumulative"
        )
        stats.print_stats(40)
        TEST_LOGGER.info(
            f"stats for dataframe of shape: {full_df.shape} are \n{string_output_stream.getvalue()}"
        )

        assert col_prof._trackers["hist"] is not None
        TEST_LOGGER.info(
            f"Benchmark was using the following trackers {col_prof._trackers.keys()}"
        )
