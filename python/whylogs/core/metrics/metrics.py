...
class MetricConfig:
    ...
    lower_case: bool  # Convert Unicode characters to lower-case before counting Unicode ranges
    normalize: bool  # Unicode normalize strings before counting Unicode ranges
    ...
    def __init__(
        ...
        self.__dict__["hll_lg_k"] = hll_lg_k if hll_lg_k is not None else conf.hll_lg_k
        self.__dict__["kll_k"] = kll_k if kll_k is not None else conf.kll_k
        self.__dict__["fi_lg_max_k"] = fi_lg_max_k if fi_lg_max_k is not None else conf.fi_lg_max_k
        self.__dict__["fi_disabled"] = fi_disabled if fi_disabled is not None else conf.fi_disabled
        self.__dict__["track_unicode_ranges"] = (
            track_unicode_ranges if track_unicode_ranges is not None else conf.track_unicode_ranges
        )
        self.__dict__["large_kll_k"] = large_kll_k if large_kll_k is not None else conf.large_kll_k
        self.__dict__["kll_k_large"] = kll_k_large if kll_k_large is not None else conf.kll_k_large
        self.__dict__["unicode_ranges"] = unicode_ranges if unicode_ranges is not None else dict(conf.unicode_ranges)
        self.__dict__["lower_case"] = lower_case if lower_case is not None else conf.lower_case
        self.__dict__["normalize"] = normalize if normalize is not None else conf.normalize
        self.__dict__["max_frequent_item_size"] = (
            max_frequent_item_size if max_frequent_item_size is not None else conf.max_frequent_item_size
        )
        self.__dict__["identity_column"] = identity_column if identity_column is not None else conf.identity_column
