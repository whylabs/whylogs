...
from typing import Dict, Generic, List, Tuple, Optional, Any, TypeVar
...
class PipeSignaler(th.Thread, Generic[T]):
    ...
    def __init__(self) -> None:
        ...
        self.queue = FasterQueueWrapper[Tuple[str, Optional[Exception], Optional[T]]](
            self._queue_config
        )
    ...
    def signal_many(
        self, results: List[Tuple[str, Optional[Exception], Optional[T]]]
    ) -> None:
            ...
            try:
                ...
                for message_id, exception, data in messages:
                    ...
                    if future is not None:
                        self._logger.debug(
                            f"Setting result for message id {message_id} {exception}"
                        )
