...
from whylogs.core.stubs import pd, np
...
def log_batch_ranking_metrics(
    ...
    formatted_data["count_at_k"] = formatted_data[relevant_cols].apply(
        lambda row: sum([1 if pred_val in row[target_column] else 0 for pred_val in row[prediction_column][:k]]), 
        axis=1
    )

    formatted_data["count_all"] = formatted_data[relevant_cols].apply(
        lambda row: sum([1 if pred_val in row[target_column] else 0 for pred_val in row[prediction_column]]), 
        axis=1
    )
    ...
    for ki in range(1, k + 1):
        ki_result = formatted_data[relevant_cols].apply(
                lambda row: sum([1 if pred_val in row[target_column] else 0 for pred_val in row[prediction_column][:ki]]), 
                axis=1
            ) / ki
        if ki == 1:
    ...
    result = result.merge(log(row={"mean_average_precision_k_"+str(k): mAP_at_k,
    ...
    # dist_columns: ["mean_average_precision_k_"+str(k), "accuracy_k_"+str(k), "mean_reciprocal_rank"], 
    # single_val_columns: ["precision_k_"+str(k), "recall_k_"+str(k), "top_rank", "average_precision_k_"+str(k)]