...
class JsonStore(ConfigStore):
    ...
    def propose_version(self, config: str, new_version: Version, note: Optional[str] = None) -> None:
        ...
        ValueError if new_version <= latest
        """
        if new_version != Version("0.0.0"):
            logger.warning("JsonStore does not currently support versioning, overwriting version 0.0.0")

        self._proposed = config
    ...
    def commit_version(self, new_version: Version) -> None:
        ...
        self._client._write_config(self._proposed, self._what)
