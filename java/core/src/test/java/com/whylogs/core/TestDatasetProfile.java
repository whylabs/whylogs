...
public class TestDatasetProfile {
    ...
    private DatasetProfile customTimeZone(){
        return new DatasetProfile(Optional.of(defaultSchema()), Optional.ofNullable(creationTime), Optional.ofNullable(datasetTime));
    }
    ...
    public void testDatasetTimes(){
        ...
        Assert.assertEquals(profile.getCreationTimestamp().getEpochSecond(), creationTime.getEpochSecond());
        Assert.assertEquals(profile.getDatasetTimestamp().getEpochSecond(), datasetTime.getEpochSecond());
    ...
    public void testTrackNullDate(){
        ...
        Assert.assertEquals(profile.getColumns().get("test").getCachedValues(), 1);
        Assert.assertEquals(profile.getTrackCount(), 1);
    }
